...
class PyDatasus:
    ...
    def get_db_partial(self, *args):
        """Baixa os arquivos db* a partir de um csv existente."""

        for i in args:
            if i != 'pass':
                self.__platform(i)
                self.__sep_db_partial(i)
    ...
    def __sep_db_complete(self, banco, d):
        """Faz uma separação da variavel e baixa todos os arquivos que forem
    ...
    def __sep_db_partial(self, banco):
        """Confere as expressões regulares e baixa somente os bancos completos
        referente um determinado ano.
        """
        if path.isfile(self.path_files_csv + banco + '.csv'):
            with open(self.path_files_csv + banco + '.csv') as f:
                data = f.readlines()
