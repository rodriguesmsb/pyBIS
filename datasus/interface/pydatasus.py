...
class PyDatasus:
    ...
    def get_db_from_memory(self, *args):
        ...
        for x in branch:
            if 'DIR' in x:
                self.__sep_csv(x.split()[3])
                                        ...
                                        '.dbf', '.dbr')):
                self.f.write(
                        '{},{},{} KB,{}\n'.format(self.__page.pwd(),
                            x.split()[3], int(x.split()[2])/1000, x.split()[0]))
            else:
                pass
        ...
        for x in branch:
            if 'DIR' in x:
                ...
                if path.isfile(self.path_files_db+d+'/'+x.split()[3]):
                    ...
                    self.__page.retrbinary('RETR ' + x.split()[3]\
                    ,open(self.path_files_db+d+'/'+ x.split()[3]\
                    ,'wb').write)
        self.__page.cwd('..')
